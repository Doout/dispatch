...
export function DeploymentRuntime({ deployment, section }: { deployment: Deployment; section: "topology" | "values" | "manifests" }) {
  ...
  const values = useMemo(() => (showAll ? data?.values ?? [] : data?.valuesAnalyzed ? data.chartValues ?? [] : []).filter((item) => item.path.toLowerCase().includes(query.trim().toLowerCase())) ?? [], [data, query, showAll]);
  ...
  return <section className="runtime-panel">
    ...
    {section === "topology" ? <><TopologyCanvas topology={data.topology} label="Deployment runtime topology" isNodeSelectable={(node) => inspectableKinds.has(node.kind)} onNodeSelect={setSelectedNode} />{selectedNode && <ResourceInspector deploymentID={deployment.id} node={selectedNode} onClose={() => setSelectedNode(null)} />}</> : <>
      ...
      {values.length ? <div className="applied-values"><div className="applied-values-head"><span>Path</span><span>Value</span></div>{values.map((item) => <div key={item.path}><code>{item.path}{item.source && <small className="value-origin">{item.source}</small>}</code><div className="value-content"><code className={item.redacted ? "redacted" : ""}>{renderValue(!item.redacted && item.renderedValue !== undefined ? item.renderedValue : item.value)}</code>{!item.redacted && item.renderedValue !== undefined && <><small className="value-origin">Rendered with Helm tpl · preview</small><details className="value-raw"><summary>Raw value</summary><code>{renderValue(item.value)}</code></details></>}</div></div>)}</div> : <p className="runtime-empty">{!showAll && !data.valuesAnalyzed ? "Chart values are unavailable for this deployment." : "No matching values."}</p>}
    </>}
  </section>;
}
