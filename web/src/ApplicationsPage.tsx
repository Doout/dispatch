import { ApplicationDeploymentLink, ApplicationOwnerLabel, useDeploymentCatalog } from "./deployments/DeploymentCatalog";
...
export function ApplicationsPage({ overview, section, applicationID, configurationSourceID, creating, onToggleCreate, onChanged, onDeploy, onDelete, onDeleteGroup, onNavigate, onOpenTopology = () => {}, onOpenConfigurationTopology = () => {}, onOpenWorkflowStage = () => {}, onOpenDeploymentManifests = () => {}, onOpenDeployment, onCloseTopology = () => {} }: { overview: Overview; section: ApplicationSection; applicationID?: string; configurationSourceID?: string; creating: boolean; onToggleCreate: () => void; onChanged: () => Promise<void>; onDeploy: (appID: string) => void; onDelete: (application: AppModel) => void; onDeleteGroup: (group: PreviewGroup) => void; onNavigate: (view: View) => void; onOpenTopology?: (resource: WorkflowResource) => void; onOpenConfigurationTopology?: (source: ConfigSource) => void; onOpenWorkflowStage?: (applicationID: string, stageName: string) => void; onOpenDeploymentManifests?: (deploymentID: string) => void; onOpenDeployment?: (id: string) => void; onCloseTopology?: () => void }) {
  ...
  const openStage = (applicationID: string, stageName: string) => {
    const item = catalog.items.find(item => item.resourceId === applicationID && item.environment === stageName);
    const deployment = item?.current ?? item?.latest;
    if (deployment && onOpenDeployment) onOpenDeployment(deployment.id);
    else onOpenWorkflowStage(applicationID, stageName);
  };
  ...
  if (configurationSourceID) {
    ...
    return <ConfigurationTopologyPage source={source} overview={overview} onBack={onCloseTopology} onOpenResource={onOpenTopology} onOpenStage={openStage} />;
  }
...
function ApplicationInventory({ onOpenDeployment, overview, onDeploy, onSync, onServices, onHooks, onValues, onEditGroup, onEditConfig, onOpenWorkflow, onOpenTopology, onOpenConfigurationTopology, onOpenWorkflowStage, onOpenDeploymentManifests, onDelete, onDeleteGroup, onChanged, hasReadyServer, hasProject, onNavigate }: { onOpenDeployment?: (id: string) => void; overview: Overview; onDeploy: (id: string) => void; onSync: (application: AppModel) => void; onServices: (application: AppModel) => void; onHooks: (application: AppModel) => void; onValues: (application: AppModel) => void; onEditGroup: (group: PreviewGroup) => void; onEditConfig: (source: ConfigSource) => void; onOpenWorkflow: (resource: WorkflowResource) => void; onOpenTopology: (resource: WorkflowResource) => void; onOpenConfigurationTopology: (source: ConfigSource) => void; onOpenWorkflowStage: (applicationID: string, stageName: string) => void; onOpenDeploymentManifests: (deploymentID: string) => void; onDelete: (application: AppModel) => void; onDeleteGroup: (group: PreviewGroup) => void; onChanged: () => Promise<void>; hasReadyServer: boolean; hasProject: boolean; onNavigate: (view: View) => void }) {
  ...
  return <section id="application-resources" aria-labelledby="application-resources-title"><h2 className="sr-only" id="application-resources-title">Configured resources</h2>{error && <p className="form-error application-inventory-error" role="alert">{error}</p>}<div className="resource-table-wrap application-inventory-table-wrap"><table className="resource-table application-inventory-table"><thead><tr><th>Name</th><th>Type</th><th>Source</th><th>Target</th><th>Status</th><th className="actions-head"><span className="sr-only">Options</span></th></tr></thead><tbody>
    ...
    {applications.map((application) => {
      ...
      return <tr key={application.id}><td data-label="Name"><strong>{application.name}</strong><small>{project}</small>{!application.template && <ApplicationOwnerLabel appId={application.id}/>}{!application.template && <ApplicationDeploymentLink appId={application.id} onNavigate={route => route.deploymentID && onOpenDeployment?.(route.deploymentID)} />}</td><td data-label="Type"><strong className="cell-secondary-heading">{type}</strong><small>{method}</small></td><td data-label="Source"><span className="truncate-cell" title={source}>{source}</span><small className="truncate-cell" title={sourceDetail}>{sourceDetail}</small></td><td data-label="Target">{target}</td><td data-label="Status"><span className={`status-label ${application.state}`}><i />{application.state}</span></td><td className="row-actions">{(canDeployApplication || canConfigureApplication || !application.template) && <RowActionMenu name={application.name}>
        ...
        {!application.template && <MenuAction icon={<SlidersHorizontal size={16} />} label={application.buildType === "helm" ? "Sync and drift" : "Checks"} onClick={() => onSync(application)} />}
