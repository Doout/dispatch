...
RUN --mount=type=cache,target=/root/.npm --mount=type=cache,target=/pnpm/store \
    npm install --global pnpm@11.18.0 && pnpm install --frozen-lockfile --store-dir /pnpm/store
COPY web/ ./
RUN pnpm build
