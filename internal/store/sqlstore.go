...
func (s *SQLStore) CreatePrivateNetwork(ctx context.Context, item core.PrivateNetwork) error {
	...
	_, err = s.db.ExecContext(ctx, s.q(`INSERT INTO private_networks(id,name,driver,laneway_application_id,laneway_installation_id,laneway_network_id,config,details,token_hash,encrypted_credentials,state,last_verified_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`),
		item.ID, item.Name, item.Driver, nullableString(item.LanewayApplicationID), nullableString(item.LanewayInstallationID), nullableString(item.LanewayNetworkID), string(config), string(details), item.TokenHash, item.EncryptedCredentials, item.State, nullTime(item.LastVerifiedAt), stamp(item.CreatedAt), stamp(item.UpdatedAt))
	return err
}
...
func (s *SQLStore) UpdatePrivateNetwork(ctx context.Context, item core.PrivateNetwork) error {
	...
	result, err := s.db.ExecContext(ctx, s.q(`UPDATE private_networks SET name=?,driver=?,laneway_application_id=?,laneway_installation_id=?,laneway_network_id=?,config=?,details=?,token_hash=?,encrypted_credentials=?,state=?,last_verified_at=?,updated_at=? WHERE id=?`),
		item.Name, item.Driver, nullableString(item.LanewayApplicationID), nullableString(item.LanewayInstallationID), nullableString(item.LanewayNetworkID), string(config), string(details), item.TokenHash, item.EncryptedCredentials, item.State, nullTime(item.LastVerifiedAt), stamp(item.UpdatedAt), item.ID)
	return changed(result, err)
}
...
func (s *SQLStore) ListPrivateNetworks(ctx context.Context) ([]core.PrivateNetwork, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id,name,driver,laneway_application_id,laneway_installation_id,laneway_network_id,config,details,token_hash,encrypted_credentials,state,last_verified_at,created_at,updated_at FROM private_networks ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *SQLStore) GetPrivateNetwork(ctx context.Context, id string) (core.PrivateNetwork, error) {
	item, err := scanPrivateNetwork(s.db.QueryRowContext(ctx, s.q(`SELECT id,name,driver,laneway_application_id,laneway_installation_id,laneway_network_id,config,details,token_hash,encrypted_credentials,state,last_verified_at,created_at,updated_at FROM private_networks WHERE id=?`), id))
	if errors.Is(err, sql.ErrNoRows) {
		return item, ErrNotFound
	}
	return item, err
...
func (s *SQLStore) GetPrivateNetworkByLaneway(ctx context.Context, applicationID, networkID string) (core.PrivateNetwork, error) {
	item, err := scanPrivateNetwork(s.db.QueryRowContext(ctx, s.q(`SELECT id,name,driver,laneway_application_id,laneway_installation_id,laneway_network_id,config,details,token_hash,encrypted_credentials,state,last_verified_at,created_at,updated_at FROM private_networks WHERE laneway_application_id=? AND laneway_network_id=?`), applicationID, networkID))
	if errors.Is(err, sql.ErrNoRows) {
		return item, ErrNotFound
	}
	return item, err
...
func scanPrivateNetwork(row scanner) (core.PrivateNetwork, error) {
	...
	var applicationID, installationID, networkID sql.NullString
	var config, details, created, updated string
	var verified sql.NullString
	err := row.Scan(&item.ID, &item.Name, &item.Driver, &applicationID, &installationID, &networkID, &config, &details, &item.TokenHash, &item.EncryptedCredentials, &item.State, &verified, &created, &updated)
	if err != nil {
		return item, err
	}
	if err := json.Unmarshal([]byte(config), &item.Config); err != nil {
	...
	item.LanewayApplicationID, item.LanewayInstallationID, item.LanewayNetworkID = applicationID.String, installationID.String, networkID.String
	item.CredentialsConfigured = item.EncryptedCredentials != ""
	item.LastVerifiedAt = parseNullTime(verified)
	item.CreatedAt, item.UpdatedAt = parseTime(created), parseTime(updated)
	return item, nil
